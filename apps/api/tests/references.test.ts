...
describe("/categories", () => {
  it("requires auth for any operation", async () => {
    await request(app).get(apiPath("/categories")).expect(401);
  });
  ...
  it("only admin can create", async () => {
    ...
    const res = await request(app)
      ...
      .send({ name: "Test", icon: "🧪", sortOrder: 99 })
      .expect(201);
  ...
  it("admin can update + soft delete", async () => {
    const admin = await loginAs("admin");
    ...
    await request(app)
      .patch(apiPath(`/categories/${created.body.id}`))
      .set(...authHeader(admin.accessToken))
      .send({ active: false })
      .expect(200);

    await request(app)
      .delete(apiPath(`/categories/${created.body.id}`))
      .set(...authHeader(admin.accessToken))
      .expect(200);

    await request(app)
      .get(apiPath(`/categories/${created.body.id}`))
      .set(...authHeader(admin.accessToken))
      .expect(404);
  });
});
